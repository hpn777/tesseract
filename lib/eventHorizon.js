...
class EventHorizon{
    ...
    registerTesseract(newTesseract) {
        ...
        resolvedColumns.forEach(column => {
            ...
            if (childrenTable) {
                childrenTable.off('dataUpdated dataRemoved')
                childrenTable.on('dataUpdated dataRemoved', data => {
        ...
        this.tesseracts.forEach(tesseract => {
            ...
            if(resolved.length) {
                newTesseract.off('dataUpdated dataRemoved')
                newTesseract.on('dataUpdated dataRemoved', data => {
	...
	createTesseractFromSession(name, session) {
		...
		if(session.config.groupBy){
			...
			session.on('dataUpdated', (data) => {
				...
				mapIds(idsCache, data.removedIds)
				debouncedClearQueue()
			})
		}
		else {
	...
	createSession(parameters) {
		...
		if (tesseract) {
			...
			return this.sessions.get(parameters.id)
		}
		else{
			throw 'Ouch!!!'
		}
