...
class Tesseract extends Model {
	...
	constructor({id, idProperty = 'id', resolve, columns, clusterSync}) {
		...
		this.refresh = _.throttle(() => {
			...
			this.trigger('update', this.dataCache)
		}, 100)
	...
	add(data, disableClusterUpdate) {
        ...
        if (this.clusterSync && !disableClusterUpdate) {
			...
			if (addedRows.length) {
				this.trigger('update', addedRows, disableClusterUpdate)
			}
		}
	}
	...
	update(data, reset, disableClusterUpdate) {
		...
		if (reset) {
            if (this.clusterSync && !disableClusterUpdate) {
				...
				self.trigger('update', this.dataCache, true)
			}
		}
		else if (data) {
			if (this.clusterSync && !disableClusterUpdate) {
				...
				if (updatedRows.length) {
					this.trigger('update', updatedRows, disableClusterUpdate)
				}
			}
		}
	...
	clear(disableClusterUpdate) {
		if (this.clusterSync && !disableClusterUpdate) {
            this.trigger('remove', this.dataCache)
        }
