            ...
            _(data).groupBy(x => x[currentGroup.dataIndex])
                .forEach((childRows, groupKey) => {
                    if (childRows.length >= 1) {
                        var tempAgregatedRow = generateSummaryRow(childRows, columns, currentGroup.dataIndex, groupKey, branchPathCopy, parent);
                        tempAgregatedRow[idProperty] = groupIdPrefix + '/' + groupKey;
                        var newgroupBy = groupBy.slice(1, groupBy.length);
                        tempAgregatedRow.children = groupData(columns, childRows, newgroupBy, includeLeafs, tempAgregatedRow[idProperty], branchPath.slice(0, branchPath.length), tempAgregatedRow);
                        response.push(tempAgregatedRow);
                    }
                })
        }
        else {
            var tempAgregatedRow = generateSummaryRow(data, columns, currentGroup.dataIndex, currentGroup.dataIndex, 'All',branchPathCopy, parent);
            tempAgregatedRow[idProperty] = currentGroup.dataIndex;
            var newgroupBy = groupBy.slice(1, groupBy.length);
            tempAgregatedRow.children = groupData(columns, data, newgroupBy, includeLeafs, currentGroup.dataIndex, branchPath.slice(0, branchPath.length), tempAgregatedRow, idProperty);
...
let groupSelectedData = (
    ...
    if (!currentGroup) {
        ...
        if (currentGroup.dataIndex != 'All') {
                ...
                .forEach((childRows, groupKey) => {
                    ...
                    if (childRows.length >= 1) {
                        ...
                        if (processChildren) {
                            ...
                            if(subData.length){
                                var tempAgregatedRow = generateSummaryRow(subData, columns, currentGroup.dataIndex, groupKey, branchPathCopy, parent, idProperty);
                                tempAgregatedRow[idProperty] = groupIdPrefix + '/' + groupKey;
                                var newgroupBy = groupBy.slice(1, groupBy.length);
            ...
            var tempAgregatedRow = generateSummaryRow(data, columns, currentGroup.dataIndex, currentGroup.dataIndex, branchPathCopy, parent, idProperty);
            tempAgregatedRow[idProperty] = currentGroup.dataIndex;
            var newgroupBy = groupBy.slice(1, groupBy.length);
            tempAgregatedRow.children = groupSelectedData(
                columns,
...
let createSessionProxyConfig = (getTesseract, allColumns, selectedColumns) => {
    ...
    allColumns.forEach((item, index) => {
        if(selectedColumns.filter(x => x.name === item.name).length === 1){
            if(item.resolve || typeof item.value === 'function'){
...
function applyOrder(sort, data) {
    ...
    if(testItem){
        ...
        sort.forEach((item, index) => {
            ...
            if(typeof testItem[item.property] === 'string'){
                if(item.direction.toLowerCase() === 'asc')
                    comparerSource += `a.${item.property}.localeCompare(b.${item.property})`
                else
                    comparerSource += `b.${item.property}.localeCompare(a.${item.property})`
            }
            else{
                if(item.direction.toLowerCase() === 'asc')
                    comparerSource += `a.${item.property} - b.${item.property}`
                else
                    comparerSource += `b.${item.property} - a.${item.property}`
            }
        })
