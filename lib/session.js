 ...
 * events: dataUpdated, dataRemoved
 */
class Session extends Model {
	...
	constructor(model, options) {
		...
		this.tesseract.on('dataUpdated', (data, disableClusterUpdate) => {
			...
			if (filters.length) {
                data.forEach(item => {
                    ...
                    if (filters.applyFilters(item)) {
						...
						if (this.dataMap[item.raw[idIndex]]) {
							removedIds.push(tempId)
							removedRows.push(this.dataMap[item.raw[idIndex]])
		...
		this.tesseract.on('dataRemoved', (data, disableClusterUpdate) => {
			let removedIds = []
			let removedRows = []
			data.forEach(item => {
	...
	getData(request) {
		...
		if (this.requireFiltring || this.requireSorting || config.start || config.limit) {
			...
			var updateDataCache = false

			if (this.requireFiltring) {
				query = _((this.tesseract.dataCache))
					.filter(x => !x.removed && this.filters.applyFilters(x))
				this.requireFiltring = false
				this.dataCache = query.map(x => new this.defaultObjDef(x.array)).value()
				this.dataMap = query.reduce( (acc, item)=>{
				...
				if(this.hasGarbage){
					query = query.filter(x => !x.removed)
				}
			}
			...
			if(this.hasGarbage) {
				var k = 0
				this.dataCache = this.dataCache.filter(x => !x.removed)
				this.dataMap = _(this.dataCache).reduce((acc, item)=>{
					acc[item.raw[this.idIndex]] = item
					return acc
				}, {})
